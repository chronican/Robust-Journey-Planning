...
By analyzing wrangled dataset from SBB corporation, we obtain our data on which we perform operation. 

The core part of this project is a algorithm which searches the shortest path given a certainty demand. Any route which yields a completion probability lower than the demand will be ruled out. In essence, we made modification to the RAPTOR algorithm.

To be more specific, RAPTOR is based on the width-first search methodology, we first implement this. However, the RAPTOR algorithm is not tailored to our requirement we necessitates estimation of path completion probability. To tackle this, we redesign the algorithm to incorporate the computation of probability with a depth-first search algorithm. In this way, when searching in all the choices, an entire path can be taken into consideration, which cannot be achieved by width-depth-based method.

Our approach is well-documented in final.ipynb

Project Structure

In final.ipynb:
1) Data cleaning and wrangling: To speed up operation, we put a lot of data into local space.
2) Predictive model: based on SBB data, we calculate the probability of a vehicle arriving on time, taking multiple factors into consideration
3) Recommendation algorithm
4) Validation: 
	i)  compare with google maps' recommendation
	ii) check the correctness of our path-completion probability calculation
	iii)check that our recommended path are actually planned by SBB
5) UI: iPywidget-based UI design

In predictive_analysis.ipynb:
Jusitify our choice of factors based on which the on-time probability is calculated

Run the algorithm:

- 1. run all the cells in the final.ipynb in order
  2. In the beginning, please remember to change the `usename` in spark to be the `username` in  local
  3. In the last part is our UI, run the cell, and it will show an user interface 
